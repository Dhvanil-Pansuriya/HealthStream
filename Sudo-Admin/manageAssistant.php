  ...
  <div class="container my-5 p-5 ">
    <div class="jumbotron container border border-danger text-dark border-2  my-5 p-5 rounded">
      ...
      <main class="container my-4 ">
        <h1 class="text-center">Add Assistant</h1>
        <div class="container" style="max-width: 50rem;">
          <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <div class="mb-3">
              <label for="exampleInputEmail1" class="form-label">Assistant Name</label>
            ...
            <button type="submit" name="signup_button" class="btn btn-outline-dark my-3 px-5">Add Assistant</button>
          </form>
        </div>
        <h1 class="text-center my-5">Remove Assistant</h1>
        <div class="container text-center" style="max-width: 50rem;">

          <a href="./kickoutAssistant.php" class="btn btn-dark ">Remove Assistant</a>
        </div>
      </main>
      ...
      if (isset($_POST['signup_button'])) {
        ...
        $sql = "SELECT name FROM assistant WHERE name = '{$username}'";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
          echo '<div class=" container alert alert-danger alert-dismissible fade show" role="alert">
                            <strong>Assistant </strong> Name Already taken by other !!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                          </div>';
        } else {
          $sql1 = "SELECT email FROM assistant WHERE email = '{$email}'";
          $result1 = mysqli_query($conn, $sql1);

          if (mysqli_num_rows($result1) > 0) {
            echo '<div class=" container alert alert-danger alert-dismissible fade show" role="alert">
                            <strong>Assistant </strong> Email Already taken by other !!
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                          </div>';
          } else {
            $sql2 = "INSERT INTO `assistant`(`name`, `email`, `phonenumber`, `password`) VALUES ('{$username}','{$email}','{$phonenumber}','{$password}') ";
            if (mysqli_query($conn, $sql2)) {
              echo '<div class=" container alert alert-success alert-dismissible fade show" role="alert">
                            <strong>Assistant </strong> add Successfully 
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                          </div>';
